    ...
    if (from){
        ...
        if (type){
            breadcrumb = [
                ...
                { label: `${type.charAt(0).toUpperCase() + type.slice(1)} type` , path: from.pathname + from.search }
            ]
        }
        else if (generation){
            breadcrumb = [
                ...
                { label: `Generation ${generation}` , path: from.pathname + from.search }
            ]
        }
        else{
            breadcrumb = [
                { label: 'Accueil', path: '/' },
                { label: 'Full Pokedex', path: '/pokedex' }
            ]
        }
    }
    else{
        ...
        <div className="pokemon-header">
            <div className="breadcrumb">            
                {breadcrumb.map((crumb, index) => (
                    <span key={index} className="breadcrumb-element">                            
                        <Link to={crumb.path}>{crumb.label}</Link>
                        <svg className="separator" viewBox="0 0 330 330">
            ...
            <div className="header-title">
                <svg /*onClick={handleBack}*/ viewBox="0 0 980 978.94">
