...
function extractEvolutions(chain) {
    ...
    const traverse = (node) => {
        evolutions.push({
            ...
            url: node.species.url, // ex: https://pokeapi.co/api/v2/pokemon-species/487/
        });
        ...
        const fetchData = async () => {
            ...
            const fullEvolutions = await Promise.all(
                evolutionNames.map(async ({ url }) => {
                    // On extrait l'ID de l'espèce depuis l'URL
                    const speciesId = url.split("/").filter(Boolean).pop();
                    // On l’utilise pour fetch les données du Pokémon (pas de risque d’erreur sur le nom)
    ...
    return(
        <div className="pokemon-detail" style={{ "--bg-color": bgColor, "--type-color" : typeColor, "--type2-color" : type2Color}}>
            <div className="pokemon-header">
                <svg onClick={() => navigate("/")} viewBox="0 0 980 978.94">
            ...
            <div className="pokemon-navigation">
                {pokemon.id !== 1 && (
                    <Link to={`/pokemon/${pokemon.id - 1}`} className="left-arrow">
                ...
                {pokemon.id !== 898 && (
                    <Link to={`/pokemon/${pokemon.id + 1}`} className="right-arrow">
                        <svg viewBox="0 0 330 330">
                            <path id="XMLID_92_" d="M111.213,165.004L250.607,25.607c5.858-5.858,5.858-15.355,0-21.213c-5.858-5.858-15.355-5.858-21.213,0.001
                            l-150,150.004C76.58,157.211,75,161.026,75,165.004c0,3.979,1.581,7.794,4.394,10.607l150,149.996
                            C232.322,328.536,236.161,330,240,330s7.678-1.464,10.607-4.394c5.858-5.858,5.858-15.355,0-21.213L111.213,165.004z"/>
