...
function FavoritesPage() {
    ...
    const showLoader = isLoading && debouncedSearch === "";
            ...
            <div className="pokedex-container">
                {filteredPokemons.map(pokemon => (
                    <PokeCard
                        ...
                        filteredIds={JSON.parse(localStorage.getItem('favorites'))}
                        from={location}
                        onFavoriteToggle={fetchFavorites}
                    />
                ))}
            </div>
            {!showLoader && filteredPokemons.length === 0 && (
                <p className="no-result">No favorite Pokemon found yet 😢</p>
            )}
            {isLoading && 
